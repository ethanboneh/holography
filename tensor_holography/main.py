...
class TensorHolographyModel():
    ...
    def evaluate(self, eval_params):
        ...
        os.makedirs(eval_params["output_path"], exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input-format', default="RGBD", type=str, help='Input format')
    parser.add_argument('--supervision-format', default="Single View", type=str, help='Supervision format')
    ...
    training_params = {
        ...
        "supervision_format": opt.supervision_format,
    }

    # determine input dimension based on format
    match opt.input_format:
        ...
        case "RGB":
            input_dim = 3
        case "Light Field":
            input_dim = 25 * 3  # 25 views * 3 channels (RGB)
        case _:
            raise ValueError(f"Unknown input format: {opt.input_format}")

    match opt.supervision_format:
        case "Single View":
            output_dim = 6
        case "Focal Stack":
            output_dim = 22 # size of deepfocus focal stack
        case "Light Field":
            output_dim = 25 * 3  # 25 views * 3 channels (RGB)
        case _:
            raise ValueError(f"Unknown supervision format: {opt.supervision_format}")

    # model parameters
    model_params = {
        "name": opt.model_name,                                       # model name
        "input_dim": input_dim,
        "output_dim": output_dim,                                  # amplitude+phase
